...
class shop(ImWeb) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://baumeo.net'
		...
		#inline_header_normal > div > div.inline-inside._inline-inside > div > div > div > div > div > ul > div.viewport-nav.desktop._main_menu > li > ul > li > a
		
		#self.C_CATEGORY_VALUE = 'body > div > div > div > div.owl-stage-outer > div > div > div > div > div > div > div > h5 > span > a'
		self.C_CATEGORY_VALUE = '#inline_header_normal > div > div.inline-inside._inline-inside > div > div > div > div > div > ul > div.viewport-nav.desktop._main_menu > li > ul > li > a'
		self.C_CATEGORY_IGNORE_STR = ['공지사항','FAQ','전체상품']
	...
	def get_category_value(self, product_data, page_url, soup ) :
		try :
			div_list = soup.find_all('div', {'doz_type':'text'})
			for div_ctx in div_list :
				h4_list = div_ctx.find_all('h4')
				for h4_ctx in h4_list :
					product_data.crw_category1 = h4_ctx.get_text().strip()
		except :
			pass
