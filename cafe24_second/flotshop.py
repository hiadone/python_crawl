...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'https://flotshop.com'
		...
		self.C_PRODUCT_VALUE = '#contents > div.xans-element-.xans-product.xans-product-normalpackage > div.xans-element-.xans-product.xans-product-listnormal.df-list-product > ul > li > div'
			...
			for li_ctx in li_list :
				...
				if(strong_ctx != None) :
					if(1 < len(span_ctx) ) :
						...
						if( 0 == title_name.find( '판매가')) and ( 0 < title_name.find( ':')) : product_data.crw_price = int( __UTIL__.get_only_digit( value_str ) )
						elif( 0 <= title_name.find( ':')) : 
							span_str = span_ctx[1].get_text().strip()
							if(0 <= span_str.find('할인')) or (0 <= span_str.find('이벤트')) : product_data.crw_price_sale = int( __UTIL__.get_only_digit( value_str ))
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			table_list = soup.select('#df-product-detail > div.detailArea > div.infoArea-wrap > div > div > div.df-detail-fixed-scroll > div.xans-element-.xans-product.xans-product-detaildesign > table')
