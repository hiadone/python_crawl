...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://www.laminarof.com'
	...
	def set_product_data(self , page_url, soup, product_ctx ) :
		...
		try :
			...
			title_list = soup.select('#-common > div > div > div.xans-element-.xans-product.xans-product-menupackage > div.xans-element-.xans-product.xans-product-headcategory.titleArea.sub')
			for title_ctx in title_list :
				h2_ctx = title_ctx.find('h2')
				if( h2_ctx != None ) : 
					product_data.crw_category1 = h2_ctx.get_text().strip()
					break
			...
			sold_out_ctx = product_ctx.find('img', class_='ec-admin-icon cart')
			if( sold_out_ctx == None) : product_data.crw_is_soldout = 1
