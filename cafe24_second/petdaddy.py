...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://petdaddy.co.kr'
		...
		self.C_CATEGORY_IGNORE_STR = ['Brand', '상품 후기', 'FAQ 스맥', 'FAQ 미티본']
		...
		self.C_PAGE_VALUE = '#contents_main > div.xans-element-.xans-product.xans-product-normalpaging.ec-base-paginate > ol > li > a'
		...
		self.C_PRODUCT_VALUE = '#contents_main > div.xans-element-.xans-product.xans-product-listnormal.ec-base-product > ul > li'
		...
		self.C_LAST_PAGE_VALUE = '#contents_main > div.xans-element-.xans-product.xans-product-normalpaging.ec-base-paginate > a.last'
		...
		self.C_PRODUCT_IMG_SELECTOR_CLASSNAME = 'thumbnail thumbnail_m'
	...
	def web_process_category_list_second(self):
		...
		try :
			...
			if( resp.status_code != 200 ) :
				...
				self.WEB_CATEGORY_NAME_HASH['Dog'] = ''
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			table_list = soup.select('#contents_main > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table')
			
			rtn_dict = self.get_value_in_table_two_colume( table_list, '기본 정보', 'th', 'td')
			if(rtn_dict.get('영문상품명' , -1) != -1) : 
				split_list = rtn_dict['영문상품명'].split('-')
				crw_brand.append( split_list[0].strip() )
