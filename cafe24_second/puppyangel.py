...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://puppyangel.co.kr'
		...
		self.C_CATEGORY_VALUE = '#top_category > div > div > ul > div > ul > li > a'
	...
	def set_product_data(self , page_url, soup, product_ctx ) :
		...
		try :
			...
			#
			# <li class="price_all">
			# <span class="custom ">₩49,900</span>
			# <span class="price  displaynone"><span class="strike">₩39,900</span><span class="pri">₩39,900</span></span>
			...
			if(price_all_ctx != None) :
				...
				for custom_ctx in custom_list :
					product_data.crw_price = int( __UTIL__.get_only_digit( custom_ctx.get_text().strip() ) )
					
				custom_list = price_all_ctx.find_all('span', class_='pri')
				for custom_ctx in custom_list :
					product_data.crw_price_sale = int( __UTIL__.get_only_digit( custom_ctx.get_text().strip() ) )
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			'''
			table_list = soup.select('#detail_wrap > div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table')
			...
			self.get_cafe24_text_img_in_detail_content_part( soup, product_data, '#prdDetail > div.xans-element-.xans-product.xans-product-additional > div.cont', '' )
