...
class shop(Cafe24) :    
			...
			for li_ctx in li_list :
				...
				if(strong_ctx != None) :
					if(1 < len(span_ctx) ) :
						...
						if( 0 == title_name.find( '판매가')) : product_data.crw_price = int( __UTIL__.get_only_digit( value_str ) )
						elif( 0 == title_name.find( ':')) : product_data.crw_price_sale = int( __UTIL__.get_only_digit( value_str ))
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			# 제품 상세 부분			
			self.get_cafe24_text_img_in_detail_content_part( soup, product_data, '#prdDetailContent', '' )
