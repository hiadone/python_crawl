...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://www.seoul-pet.com'
		...
		self.C_CATEGORY_VALUE = '#category > li.xans-element-.xans-layout.xans-layout-category.sub_menu > span > a'
				...
				if(self.check_ignore_category( category_ctx ) ) :
					if('href' in category_ctx.attrs ) : 
						...
						if(tmp_category_link.find('javascript') < 0 ) :
							...
							# 카테고리명 수정 "- " 삭제
							category_name = category_ctx.get_text()[2:].strip()
