...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://www.clusterround.com'
		...
		self.C_CATEGORY_VALUE = '#allStore_wrap > nav > ul > li > a'
		...
		self.C_PAGE_VALUE = '#allStore_contents > div.allStore_layout > div > div.xans-element-.xans-product.xans-product-normalpaging.ec-base-paginate.paging > ol > li > a'
		...
		self.C_PRODUCT_VALUE = '#allStore_contents > div.allStore_layout > div > div.xans-element-.xans-product.xans-product-normalpackage > div.xans-element-.xans-product.xans-product-listnormal > ul > li > div'
		...
		self.C_LAST_PAGE_VALUE = '#allStore_contents > div.allStore_layout > div > div.xans-element-.xans-product.xans-product-normalpaging.ec-base-paginate.paging > a.btn.last'
	...
	def set_product_data(self , page_url, soup, product_ctx ) :
		...
		try :
			...
			# <strong class="title displaynone"><span style="font-size:12px;color:#000000;font-weight:bold;">영문상품명</span></strong> <span style="font-size:12px;color:#000000;font-weight:bold;">Triangle stripe muffler green</span></li>
			# <li class=" xans-record-">
			# <strong class="title displaynone"><span style="font-size:12px;color:#000000;">소비자가</span></strong> <span style="font-size:12px;color:#000000;text-decoration:line-through;">12,000원</span></li>
			# <li class=" xans-record-">
			# <strong class="title displaynone"><span style="font-size:16px;color:#ff0000;font-weight:bold;">판매가</span></strong> <span style="font-size:16px;color:#ff0000;font-weight:bold;">SOLD OUT</span></li>
			# <li class=" xans-record-">
			# <strong class="title "><span style="font-size:12px;color:#000000;">REVIEW</span></strong> <span style="font-size:12px;color:#000000;">0</span></li>
			# </ul>
			##############################
			...
			for li_ctx in li_list :
				...
				if(strong_ctx != None) :
					if(1 < len(span_ctx) ) :
						...
						if( 0 == title_name.find( '소비자가')) : product_data.crw_price = int( __UTIL__.get_only_digit( value_str ) )
						elif( 0 == title_name.find( '판매가')) : 
							# 품절여부 확인
							if( 0 <= value_str.find('SOLD' )) : product_data.crw_is_soldout = 1
							else : product_data.crw_price_sale = int( __UTIL__.get_only_digit( value_str ))
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			table_list = soup.select('#prdDetail > div > div > table')
			...
			if(rtn_dict.get('제조자' , -1) != -1) : crw_brand.append( rtn_dict['제조자'] )
