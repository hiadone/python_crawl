...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://pets-plate.com'
		...
		self.C_CATEGORY_VALUE = '#header > div.category > div > div > ul > li > a'
		self.C_CATEGORY_IGNORE_STR = ['전체카테고리']
	...
	def set_product_data(self , page_url, soup, product_ctx ) :
		...
		try :
			...
			#
			#<div class="status">
			#<div class="icon">   <img src="/web/upload/custom_6.gif" alt=""></div>
			#</div>
			...
			for soldout_div_ctx in soldout_div_list :
				...
				for img_ctx in img_list :
					if('src' in img_ctx.attrs ) :
						if( 0 <= img_ctx.attrs['src'].find('custom_6.gif')) : product_data.crw_is_soldout = 1
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			table_list = soup.select('#prdInfo > div.xans-element-.xans-product.xans-product-detaildesign > table')
