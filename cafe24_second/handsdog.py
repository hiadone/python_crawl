...
class shop(Cafe24) :    
		...
		try :
			...
			for li_ctx in li_list :
				...
				if(span_ctx != None ) :
					...
					if('style' in span_ctx.attrs ) and (0 <= value_str.find('원')) :
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			'''
			table_list = soup.select('#allStore_contents > div > div > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.infoArea > div.infoAreaLay > div.info-left > div.xans-element-.xans-product.xans-product-detaildesign > table')
