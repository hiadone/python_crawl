...
특이사항
	- 펫브러쉬만 적용 카테고리만 수집함. ( http://tangleangel.co.kr/product/list.html?cate_no=51 )
...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		self.SITE_HOME = 'http://tangleangel.co.kr/product/list.html?cate_no=51'
		
		self.ORG_SITE_HOME = 'http://tangleangel.co.kr'
	...
	def process_category_list(self):
		...
		try :
			...
			self.CATEGORY_URL_HASH[self.SITE_HOME] = ''
	...
	def get_product_detail_data(self, product_data, html):
		...
		try :
			...
			table_list = soup.select('#prdDetail > div.cont.productdetail > center > div > center > table')
			
			rtn_dict = self.get_value_in_table_two_colume( table_list, '상품정보고시', 'th', 'td')
			if(rtn_dict.get('BRAND' , -1) != -1) : crw_brand.append( rtn_dict['BRAND'] )
			if(rtn_dict.get('브랜드' , -1) != -1) : crw_brand.append( rtn_dict['브랜드'] )
			if(rtn_dict.get('제조원/원산지' , -1) != -1) : 
				split_list = rtn_dict['제조원/원산지'].split('/')
				for split_data in split_list :
					crw_brand.append( split_data.strip() )
			if(rtn_dict.get('수입, 판매원' , -1) != -1) : crw_brand.append( rtn_dict['수입, 판매원'] )
			...
			# 제품 상세 부분			
			self.get_cafe24_text_img_in_detail_content_part( soup, product_data, '#prdDetail > div.cont.productdetail', '' )
