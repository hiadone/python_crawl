...
class shop(Cafe24) :    
	...
	def __init__(self) :
		...
		#self.C_PRODUCT_VALUE = '#contents1 > div > div.xans-element-.xans-product.xans-product-listnormal.ga09list > ul > li > div > div.name > div > a'
		self.C_PRODUCT_VALUE = '#contents1 > div > div.xans-element-.xans-product.xans-product-listnormal.ga09list > ul > li'
	...
	def set_product_data(self , page_url, soup, product_ctx ) :
		...
		try :
			...
			name_div_list = product_ctx.find_all('div', class_='prdpadding')

			for name_div_ctx in name_div_list :
				#
				# 상품 링크 정보 및 상품명 / 상품코드
				#
				name_strong_list = name_div_ctx.find_all('div', class_='name')
				...
				# 가격 브랜드
				#
				span_list = name_div_ctx.find_all('span')
				for span_ctx in span_list :
					if('class' in span_ctx.attrs) :
						class_name_list = span_ctx.attrs['class']
						if(len(class_name_list) == 2) :
							if(class_name_list[0] == 'price') and (class_name_list[1] == 'normal') : product_data.crw_price = int( __UTIL__.get_only_digit( span_ctx.get_text().strip() ))
